...
**Atenção a uma regra fácil de esquecer:** o chunk listado em `dependson`
precisa **ele próprio** estar com `cache: true`. Se ele não estiver, o `knitr`
avisa `code chunks must not depend on the uncached chunk` e interrompe a
renderização. Ou você cacheia a cadeia inteira, ou não cacheia nenhum elo dela.
...
::: {.callout-note}
## O cache do `knitr` precisa do pacote `codetools`

Para descobrir de quais objetos um chunk depende, o `knitr` usa o pacote
`codetools`. Ele costuma vir junto com o R (é um pacote "recomendado"), mas
algumas instalações mínimas — inclusive certos ambientes conda — não o
incluem. Se aparecer `there is no package called 'codetools'`, instale-o:
...
# Bibliotecas e configurações globais do relatório
library(knitr)
```
...
# Parâmetros da simulação — é aqui que você mexe para testar
# o critério de sucesso no fim desta seção.
set.seed(2026)
n_genes     <- 2000
n_por_grupo <- 6
n_de        <- 150
